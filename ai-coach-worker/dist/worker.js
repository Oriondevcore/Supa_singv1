...
var worker_default = {
  async fetch(request, env) {
    ...
    if (path === "/health") {
      return json({ status: "ok", service: "ai-coach", timestamp: (/* @__PURE__ */ new Date()).toISOString() });
    }
    ...
    if (path === "/interview" && method === "POST") {
      try {
        ...
        if (!session_id) {
          ...
          const aiResp2 = await env.AI.run("@cf/google/gemma-4-26b-a4b-it", {
            messages: [
...
Ask ONE open-ended question at a time. Be conversational and warm.` },
          ...
          const question2 = aiResp2.response?.trim() || `Let's start with phase one: ${firstPhase.label}. Tell me about yourself!`;
          return json({ session_id: sid, question: question2, phase: firstPhase.label });
        }
        ...
        if (nextStep >= 3) {
          ...
          const phase2 = INTERVIEW_PHASES[nextPhase];
          const aiResp2 = await env.AI.run("@cf/google/gemma-4-26b-a4b-it", {
            messages: [
              { role: "system", content: `You are conducting a friendly interview to learn about a singer. The current phase is "${phase2.label}". ${phase2.prompt}

Transition smoothly from the previous topic. Ask ONE open-ended question.` },
              { role: "user", content: `The user just answered about ${INTERVIEW_PHASES[currentPhase].label}. Move to ${phase2.label}.` }
            ],
            max_tokens: 300,
            temperature: 0.7
          });
          const question2 = aiResp2.response?.trim() || `Great! Now let's talk about ${phase2.label}. ${phase2.prompt}`;
          ...
          return json({ session_id, question: question2, phase: phase2.label });
        }
...
Analyze this performance and give feedback.` }
    ...
    if (path.match(/^\/my-songs\/(\d+)$/) && method === "DELETE") {
      ...
      try {
        ...
        if (song?.r2_key) {
          try {
            await env.RECORDINGS.delete(song.r2_key);
          } catch {
          }
        }
